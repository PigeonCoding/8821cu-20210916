...
#define PPPOE_HDR_TAG_DATA(ph) ((unsigned char *)((ph) + 1))
...
static unsigned char *__nat25_find_pppoe_tag(struct pppoe_hdr *ph, unsigned short type)
{
	...
	start_ptr = cur_ptr = PPPOE_HDR_TAG_DATA(ph);
...
static int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	memmove((PPPOE_HDR_TAG_DATA(ph) + data_len), PPPOE_HDR_TAG_DATA(ph), ntohs(ph->length));
	ph->length = htons(ntohs(ph->length) + data_len);
	memcpy(PPPOE_HDR_TAG_DATA(ph), tag, data_len);
	return data_len;
}
